...
trait Property(val value: String) extends BaseObject:
  ...
  def toICS(): String =
    if parameters.isEmpty then s"${name}:$value"
    else s"${name};${parameters.map(_.toICS()).mkString("")}:$value"
...
object Property:
  ...
  case class Last_Modified(last_modified: String)
      extends Property(last_modified)

  /** Gives Seq containing formatted DtStart and formatted DtEnd Property
    *
    * @param date
    *   date in format as in Kapten Alloc (YYYY-MM-DD) or YYYMMDD (which it will
    *   be formatted to in the method)
    * @param time
    *   time in format HHMM as an Integer
    * @param parameter
    *   parameter to be added to property; defaults to TzID parameter of
    *   Europe/Stockholm timezone
    * @return
    *   both Properties must be given
    */
  def time(
      date: String,
      time: Int,
      parameter: Option[Parameter] = Option(Parameter.TzIDParam(tzid().tzid))
  ): Seq[Property] =
    ...
    Seq(
      {
        ...
        val d = DtEnd(
          s"${date_formatted}T${"%04d".format(((time / 100).toInt + 2) * 100)}00"
        )
        if parameter.isDefined then d.addParameter(parameter.get)
        d
      }
  ...
  /** Gives formatted Location Property
    *
    * @param room
    *   which session is to be hold in
    */
  ...
  /** Gives formatted Description Property
    *
    * @param course
    *   type of course being held
    * @param group
    *   group to be tutored
    * @param room
    *   room held in; will be formatted differently if room starts with
    *   "Ambulans"
    */
  def description(course: String, group: String, room: String): Description =
    Description(s"Kurs: $course\\nGrupp: $group\\n${
        if room.startsWith("Ambulans") then "Roll" else "Rum"
      }: $room")
  ...
  /** Gives formatted Summary Property
    ...
    *   type of course being held
    * @param kind
    *   type of session, like "Resurstid" or "Labb"
    * @param room
    *   room held in; will be formatted differently if room starts with
    *   "Ambulans"
    */
  def summary(course: String, kind: String, room: String): Summary =
    Summary(
      s"($course) $kind ${if room.startsWith("Ambulans") then "som" else "i"} $room"
    )
